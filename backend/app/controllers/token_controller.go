...
func RenewTokens(c *fiber.Ctx) error {
	...
	if now < expiresRefreshToken {
		...
		// Get user by ID.
		_, err = db.GetUserByUsername(userUsername)
		if err != nil {
		...
		tokens, err := utils.GenerateNewTokens(userUsername)
		if err != nil {
		...
		errRedis := connRedis.Set(context.Background(), userUsername, tokens.Refresh, 0).Err()
		if errRedis != nil {
			// Return status 500 and Redis connection error.
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"error": true,
